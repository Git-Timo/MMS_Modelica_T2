...
//if(f_s3.s==f_s2.s){ //Stoppen, wenn Flaschenzug ganz zusammengefahren
  ...
  model Seilwinde
    ...
    parameter Modelica.SIunits.Mass m =10 "Masse der Seilwinde excl. Seil";   //um Massenträgheit der Rolle zu Berechnen
    
    Real d_Winde;
    Integer v;
    Real o, dmax, s;
    Real J_Rolle, v_Rolle; //Massenträgheit der Rolle, Rotationsgeschwindigkeit der Rolle
  ...
  equation
    ...
    if (v-1) >= 1 then     ///Berechnung des Windendurchmessers nicht als Step Funktion, sondern bisher als lineare Funktion
    ...
    v_Rolle=der(f_s1.s);  //Drehgeschwindigkeit der Rolle
    J_Rolle= 0.5*m*(Durchmesser/2)^2; //Berechneung des Massenträgheits der Rolle unter Annahme eines massiven Zylinders.
    ...
    -v=floor((m_w1.w/(2*Pi))/(b/d));    //berechnet die Anzahl der Schichten +1
    v=o;
    f_s1.F=((m_w1.M-Lagerwiderstand)/(d_Winde/2))-(J_Rolle*der(v_Rolle));   //Kraftberechnung am Port, inklusive Massenträgheitsbetrachtung
    f_s1.s=Pi*d_Winde*(m_w1.w/(2*Pi));
  ...
  equation
    ...
    m_w2.M=(m_w1.M*Uebersetzung*(Wirkungsgrad/100)) - v_w_ein*J_Eingangswelle - v_w_aus*J_Ausgangsewelle;
...
//Tst*der(UI)+UI=Kst*UT;
  ...
  package Modelle
    model Masse
      ...
      Flaschenzug.F_s f_s1 annotation(
        Placement(visible = true, transformation(origin = {0, 6}, extent = {{-48, -48}, {48, 48}}, rotation = 0), iconTransformation(origin = {2, 120}, extent = {{-20, -20}, {20, 20}}, rotation = 0)));
      parameter Modelica.SIunits.Mass m = 100 "Masse"; //Masse
      Modelica.SIunits.Acceleration g = Modelica.Constants.g_n; //Erdbeschleunigung
...
Ml= m_w1.M;
    annotation(
        Icon(graphics = {Ellipse(origin = {-21, 23}, extent = {{-29, 27}, {29, -27}}, endAngle = 360), Text(origin = {-24, 23}, fillPattern = FillPattern.Solid, extent = {{-12, 9}, {12, -9}}, textString = "Lastmoment")}));end Lastmoment;
  end Modelle;
