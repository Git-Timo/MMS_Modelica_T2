...
model Zeitgesteuert
  ...
  parameter Modelica.SIunits.Voltage U_A = 36 "Spannung zum Senken ab Zeitpunkt t2";   
//Variablen
  Real t10; 
  Real I(unit "A");
  //Konstanten
  ...
  annotation(
    ...
    Documentation(info = "<html><head></head><body><b>Spannungsquelle Zeitverlauf:</b><div><br></div><div>Mit diesem Spannungsmodell kann die Spannung über die Zeit verändert werden.&nbsp;</div><div>Damit lassen sich <b>Hebezyklen realisieren</b> (Heben, Halten Senken, Halten)</div><div><br></div><div>Dabei kann dieses Modell bei dem Beispiel: \"Flaschenzug_Heben_Senken\" frei parametriert werden.</div><div><br></div><div><span style=\"font-size: 12px;\">Folgende&nbsp;</span><b style=\"font-size: 12px;\">Parameter</b><span style=\"font-size: 12px;\">&nbsp;stehen zur Eingabe bereit:</span></div><div><span style=\"font-size: 12px;\"><br></span></div><div><b>t1</b> -&gt; entspricht der Hebezeit wie lange die Masse nach oben beschleunigt werden soll.&nbsp;</div><div><b>t2</b> -&gt; entspricht dem Zeitpunkt wie lange die Masse gehalten werden soll.</div><div><b>t3</b> -&gt; entspricht dem Zeitpunkt bis wann die Masse abgelassen werden soll. Anschließend wird die Masse auf dieser Höhe gehalten.</div><div><br></div><div><b>U_A&nbsp;</b>-&gt; Spannung die zum Ablassen der Masse eingestellt wird.</div><div><br></div><div><div style=\"font-size: 12px;\"><b>U</b>&nbsp;ist die Spannung für den Betrieb des Motors. Sie kann negativ oder positiv gewählt werden (Drehrichtung). Ein positiver Wert erzeugt eine positive Drehrichtung. Dabei ist zu beachten, dass der Motor genügend Spannung erhält, um die gewünschte Leistung abgeben zu können.</div></div><div><br></div><div><br></div><div><br></div></body></html>"));
      ...
      model Konstantspannung
        //Parameter
        parameter Modelica.SIunits.Voltage U = 48 annotation(
    ...
    package Seilwinden
      model Seilwinde
        ...
        parameter Modelica.SIunits.Torque Lagerwiderstand = 0.0 annotation(
          Dialog(group = "Trägheit und Verlust"));
        parameter Modelica.SIunits.Diameter d = 0.05 "Seildurchmesser" annotation(
          Dialog(group = "Geometrie"));
        parameter Modelica.SIunits.Length b = 0.4 "Windenbreite" annotation(
          Dialog(group = "Geometrie"));
...
Seillage sprunghaft ansteigen. In der Betriebsart \"Durchmesserkumulation\"
...
\"Durchmesserkumulation\", ein konstanter Trommeldurchmesser
angenommen. Das heißt, dass die Durchmesserkumulation nur beachtet wird, wenn
das Seil aufgewickelt wird.<font size=\"4\"><o:p></o:p></font></span></p>

<!--EndFragment-->

<!--EndFragment--><div>&nbsp;<div><div><br></div><div><b>Hinweis:</b> Die Parameter in der Gruppe Geometrie werden nur in der Betriebsart mit aktivierter Durchmesserkumulation benötigt. Ist diese Betriebsart nicht aktiviert, werden die eingetragenen Werte nicht beachtet, sodass sie keinerlei Einfluss auf die Simulation haben.&nbsp;</div></div></div></div></body></html>"));
      end Seilwinde;
...
//Ausgabe der Seilposition
  ...
  Documentation(info = "<html><head></head><body><b>Die Seilwinde</b>&nbsp;dient zur Umwandlung einer Drehbewegung in eine translatorische Bewegung. Durch die Bidirektionalität kann auch eine translatorische Bewegung in eine rotatorische Bewegung umgewandelt werden.&nbsp;<div><br></div><div>Das Modell besitzt zwei Ports: Ein für translatorische Bewegung (f_s) und einen für rotatorische Bewegung (m_w).<br><div><br></div><div>Folgende&nbsp;<b>Parameter</b>&nbsp;stehen zur Eingabe bereit:</div><div><br></div><div><b>Durchmesser:&nbsp;</b>Dieser Parameter beschreibt in der Einheit Meter den Durchmesser der Seiltrommel.</div><div><b>d</b>: Beschreibt den Durchmesser des Seiles in Metern. Da in der Bibliothek die Seildehnung nicht betrachtet wird, ist der Durchmesser des Seiles lastunabhängig und wird als konstant angenommen.</div><div><b>b</b>: Die Breite der Winde in Metern. Unter Berücksichtigung von d wird mit dieser Angabe in der Betriebsart Durchmesserkumulation berechnet, wie viele Umdrehungen das Seil einlagig auf die Seilrolle passt.</div><div><b>Lagerwiderstand:&nbsp;</b>Angabe des Lagerwiderstandes der Seilrolle in Newtonmeter. Damit kann ein Verlust des Antriebmomentes simuliert werden.&nbsp;</div><div><b>m</b>: Die Masse der Seilrolle in Kilogramm. Mit dieser Angabe wird die Trägheit der Seilrolle berechnet. Dabei wird angenommen, dass es sich um einen Vollzylinder handelt. Ein etwaiges Gewicht des Seiles wird in dieser Berechnung nicht beachtet.&nbsp;</div><div><b>Durchmesserkumulation</b>: Eingabe eines Wertes im booleschen Format. Bei der Eingabe des Wertes \"False\", wird ein konstanter Windendurchmesser angenommen. Wird der Parameter auf \"True\" gesetzt wird in der Simulation berücksichtigt, dass der Durchmesser der Seiltrommel im Betrieb mit zunehmend aufgewickeltem Seil wächst. Das hat direkte Auswirkungen auf die ausgegebene Seilposition. Dabei ist zu beachten, dass in dieser Betriebsart die Position nicht exakt, sondern approximiert ausgegeben wird.&nbsp;&nbsp;<span style=\"font-family: 'MS Shell Dlg 2', serif;\">Bei einer exakten Ausgabe würde das erforderliche Antriebsmoment mit den Lagen der Seiltrommel wachsen. Die Geschwindigkeit der Seilposition würde bei jeder neuen Seillage sprunghaft ansteigen.&nbsp;</span><span style=\"font-family: 'MS Shell Dlg 2', serif;\">In der Betriebsart \"Durchmesserkumulation\" wächst die Seilgeschwindigkeit und das erforderliche Antriebsmoment ab der zweiten Seillage kontinuierlich.</span></div><p style=\"margin: 0cm 0cm 0.0001pt;\"><span style=\"font-family: 'MS Shell Dlg 2', serif;\">Wird die Masse heruntergelassen, wird immer auch in der Betriebsart \"Durchmesserkumulation\", ein konstanter Trommeldurchmesser angenommen. Das heißt, dass die Durchmesserkumulation nur beachtet wird, wenn das Seil aufgewickelt wird.</span></p><div>&nbsp;<div><div><br></div><div><b>Hinweis:</b>&nbsp;Die Parameter in der Gruppe Geometrie werden nur in der Betriebsart mit aktivierter Durchmesserkumulation benötigt. Ist diese Betriebsart nicht aktiviert, werden die eingetragenen Werte nicht beachtet, sodass sie keinerlei Einfluss auf die Simulation haben.&nbsp;</div></div></div></div></body></html>"));
      end Seilwinde_Decke;
  annotation(
...
//Kraft an oberer Flasche
        ...
        annotation(
          ...
          Documentation(info = "<html><head></head><body><p style=\"font-size: 12px;\">Der Flaschenzug bietet analog zum Getriebe im Rotatorischen eine Untersetzung in das Translatorische.&nbsp;</p><div class=\"htmlDoc\"><div><br></div><div><b>Ports</b></div><div>Das Modell besitzt drei F_s Ports über diese bidirektionale Kraft und Position übergeben werden kann.&nbsp;</div><div><span style=\"font-size: 9pt; line-height: 107%; font-family: 'MS Shell Dlg 2', sans-serif;\">Ein boolescher Output dient zur
Ausgabe eines Sensorsignals. Der Sensor detektiert, ob sich die Masse bei oder
oberhalb einer bestimmten Höhe (s. Parameter)&nbsp;befindet. Detektiert der
Sensor die Masse, so wird am booleschen Ausgang ein&nbsp;</span><i>true</i>&nbsp;ausgegeben.
Ist die Masse unterhalb dieser festgelegten Höhe, so wird&nbsp;<i>false</i>&nbsp;ausgegeben.
...
<!--EndFragment--></div><div><br></div><div>Folgeende<b>&nbsp;Parameter</b>&nbsp;stehen bereit:&nbsp;</div><div><b>n</b>&nbsp;(Anzahl der Rollen): Daraus wird modellintern das Übersetzungsverhältnis des Flaschenzugs berechnet. Für n können nur ganzzahlige Werte größer 1 eingegeben werden. Grundlegend ist die an der unteren Flasche (f_s) wirkendn Kraft n mal höher als die am f_s2 -Port wirkende Kraft. Störgrößen, wie die Trägheit der unteren Flasche wirken sich auf diese grundlegende Gleichung aus. Beim Weg verhält sich der Zusammenhang ähnlich: die Positionsänderung &nbsp;der unteren Flasche n+1 mal kleiner als die Positionsänderung des f_s2 -Ports. Der Startwert ergibt sich aus der Position der oberen Flasche - der Anfangslänge des Flaschenzugs s.&nbsp;</div><div><b>s</b>&nbsp;(Anfangslänge): Die Anfangslänge beschreibt den Abstand der oberen zur unteren Flasche. Meist wird im Modell die obere Flasche mit einem Fixpunkt verbunden, sodass sich die Position der unteren Flasche aus der Höhe des Fixpunktes - s ergibt.&nbsp;</div><div><b>Zugwinkel:&nbsp;</b>Der Zugwinkel in Rad wird im Modell dafür verwendet die am f_s1-Port (obere Flasche) auftretenden Kräfte in vertikale Richtung zu berechnen. Die horizontal Wirkenden Kräfte werden im Modell vernachlässigt.</div><div><b>Sa&nbsp;</b>(Sensorabstand): Der Sensorabstand in Metern gibt den Abstand des Positionssensors bis zur oberen Flasche an. Die Funktionsweise des Sensors ist bei den Portbeschreibungen zu finden.&nbsp;</div><div><b>Flaschengewicht_unten</b>: Dieser Parameter gibt das Flaschengewicht der unteren Flasche an. Die Angabe wird im Modell benutzt um die Trägheit und das Leergewicht des Flaschenzuges zu kennen.&nbsp;</div><div><b style=\"font-size: 12px;\">Flaschengewicht_oben</b><span style=\"font-size: 12px;\">: Dieser Parameter gibt das Flaschengewicht der oberen Flasche an.&nbsp;</span><span style=\"font-size: 12px;\">Die Angabe wird im Modell benutzt um die Trägheit und das Leergewicht des Flaschenzuges zu kennen.</span></div></div></body></html>"));
...
//Berechnung der Geschwindigkeits-Variable
        ...
        annotation(
          ...
          Documentation(info = "<html><head></head><body>Die<b> Masse </b>dient im Modell meist als Objekt, welches bewegt werden soll. In der Simulation wird die Trägheit mitberücksichtigt.&nbsp;<div><b>Parametrierbar</b> ist hierbei allein die Masse in Kilogramm.&nbsp;</div><div><br></div><div>Am <b>Kraft-Weg-Port</b> können Kräfte und Positionen bidirektional übergeben werden.</div></body></html>"));
      end Masse;
...
//Berechnung der Geschwindigkeits-Variable
        ...
        annotation(
          ...
          Documentation(info = "<html><head></head><body><span style=\"font-size: 12px;\">Die</span><b style=\"font-size: 12px;\">&nbsp;Masse&nbsp;</b><span style=\"font-size: 12px;\">dient im Modell meist als Objekt, welches bewegt werden soll. In der Simulation wird die Trägheit mitberücksichtigt.&nbsp;</span><div style=\"font-size: 12px;\"><b>Parametrierba</b>r ist hierbei allein die Masse in Kilogramm.&nbsp;</div><div style=\"font-size: 12px;\"><br></div><div style=\"font-size: 12px;\">Am&nbsp;<b>Kraft-Weg-Port</b>&nbsp;können Kräfte und Positionen bidirektional übergeben werden.</div></body></html>"));
      end Bierkasten;
    annotation(
        Icon(graphics = {Polygon(fillColor = {102, 102, 102}, fillPattern = FillPattern.Solid, points = {{-100, 60}, {-60, 100}, {60, 100}, {100, 60}, {100, -100}, {-100, -100}, {-100, -84}, {-100, 60}}), Text(origin = {-35, 33}, lineColor = {221, 221, 221}, fillColor = {235, 235, 235}, fillPattern = FillPattern.Solid, extent = {{-27, 23}, {97, -77}}, textString = "Masse")}, coordinateSystem(initialScale = 0.1)),
...
//konstantes Moment an Port weitergeben
  ...
  Documentation(info = "<html><head></head><body>Das Lastmoment ermöglicht ein Testaufbau ohne Seilwinde und Gewicht. Es erzeugt ein konstantes Lastmoment an seinem m_w- Port. Das Lastmoment kann als Parameter eingegeben werden.</body></html>"));
      ...
      equation
        ...
        annotation(
          ...
          Documentation(info = "<html><head></head><body>Die <b>Boolesche_Senke</b> ist eine Senke für booleschen Werte. Sollte im Gesamtmodell ein offener Boolescher Output offen sein, kann die Senke dazu benutzt werden, diesen Wert aufzunehmen. Im Simulationsmodell hat die Senke einen optischen Charakter. Technisch wird sie (anders wie die boolesche Quelle) nicht benötigt.</body></html>"));
      end Boolesche_Senke;
  ...
  Flaschenzug.Ports.BoolOut boolOut1 annotation(
        ...
        parameter Modelica.SIunits.Time nach= 1 annotation(
          Dialog(group = "Zeitgesteuert ändern auf"));
      equation
        if time <= nach then
          boolOut1 = Aenderung;
        ...
        annotation(
          ...
          Documentation(info = "<html><head></head><body>Die Boolesche Quelle ermöglicht die Modellierungen ein boolesches Signal konstant zu halten, oder zeitgesteuert zu ändern. Das erzeugte Signal wird aus dem booleschen Output-Port ausgegeben.<div><br></div><div><b>Folgende Parameter </b>stehen zur Verfügung:</div><div><br></div><div><b>\"Eingabe\":</b> Dieser Wert nimmt der boolesche Ausgangsport von Beginn an der Simulation an</div><div><b>\"nach\":</b> soll eine Änderung des booleschen Wertes bei einem bestimmten Zeitpunkt erfolgen, so kann unter der Angabe \"nach\" der Zeitpunkt eingegeben werden.</div><div><b>\"Aenderung\":</b> Diesen Wert nimmt der boolesche Ausgangsport ab dem Zeitpunkt \"nach\" an.</div><div><br></div><div><b>Hinweis:</b></div><div>Soll keine Änderung des booleschen Wertes simuliert werden, kann entweder der Parameter \"nach\" auf einen Zeitpunkt nach dem Simulationsende, oder \"Anderung\"=\"Eingabe\" &nbsp;gesetzt werden.</div></body></html>"));
  ...
  annotation(
        ...
        Documentation(info = "<html><head></head><body>Die Sonderkomponenten-Bibliothek beinhaltet Modelle, die zum Einsatz kommen, um einzelne Modelle und deren Interaktion zu testen. Diese werden in einem kleinen, mit wenigen einzelnen Komponenten ausgestatteten Modell simuliert.&nbsp;<div><br><div>Unter <b>Beispiele/Sonderbeispiele</b> sind zwei Modelle mit Sonderkomponenten ausgerüstet.</div></div></body></html>"));
